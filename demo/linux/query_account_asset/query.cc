...
void QuerySPI::onQryAccountAsset(TFUserAccountAssetField *pAccount, TFRspInfoField *pRspInfo) {
  std::cout << __FUNCTION__ << std::endl;
  char err_msg[255];
  gbk2utf8(err_msg, 255, pRspInfo->ErrorMsg, strlen(pRspInfo->ErrorMsg));

  std::cout << pRspInfo->ErrorID << ":" << err_msg << std::endl;
  std::cout << "acc id: " << pAccount->AccountID << ", balance:" << pAccount->Balance << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  TFQryAccountAssetField af;
  af.AccountID = account_id;
  api->qryAccountAsset(&af);
