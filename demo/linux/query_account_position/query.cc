...
void QuerySPI::onQryPosition(TFUserComboPositionField *pPosition, TFRspInfoField *pRspInfo) {
  std::cout << __FUNCTION__ << std::endl;
  ...
  if (pRspInfo->ErrorID == 0 && pPosition) { std::cout << pPosition->InstrumentID << ":" << pPosition->TotalPosition << std::endl; }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "请输入您的操作员账户(operator_id)、密码、账户编码(account_id)、组合编码(combo_id)，以空格分隔:" << std::endl;
  std::cin >> operator_id >> password >> account_id >> combo_id;
  std::cin.get(); // Omit \n

  // TF_PT_STPStockQry - STP股票查询柜台类型，参阅TFDataType.h
  ...
  TFQryPositionField pf;
  pf.AccountID = account_id;
  pf.ComboID = combo_id;
